...
void Sync_update_situation(Situation *situation, const Anima *animas) {
  ...
  for (size_t id = 0; id < situation->animas.count; ++id) {
    atomic_store(&situation->animas.states[id].direction_actual,
                 atomic_load(&animas[id].smt.situation.animas.states[id].direction_actual));

    atomic_store(&situation->animas.states[id].location,
                 atomic_load(&animas[id].smt.situation.animas.states[id].location));

    atomic_store(&situation->animas.states[id].movement_pattern,
                 atomic_load(&animas[id].smt.situation.animas.states[id].movement_pattern));

    atomic_store(&situation->animas.states[id].status,
                 atomic_load(&animas[id].smt.situation.animas.states[id].status));
  }
}
