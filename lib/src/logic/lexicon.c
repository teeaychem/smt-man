...
void lexicon_ctor(lexicon_s *self) {
  ...
  *self = (lexicon_s){
      .anima = {
          .consts = malloc(self->anima.count * sizeof(*self->anima.consts)),
          .names = malloc(self->anima.count * sizeof(*self->anima.names)),
          .testers = malloc(self->anima.count * sizeof(*self->anima.testers)),
      },
  };
}

void lexicon_dtor(lexicon_s *self) {
  free(*self->anima.testers);
  free(*self->anima.names);
  free(*self->anima.consts);
}
...
void lexicon_setup_path(lexicon_s *lexicon, Z3_context ctx) {
  ...
  {
    lexicon->path.names[0] = Z3_mk_string_symbol(ctx, "o");
    lexicon->path.names[1] = Z3_mk_string_symbol(ctx, "a");
    lexicon->path.names[2] = Z3_mk_string_symbol(ctx, "b");
    lexicon->path.names[3] = Z3_mk_string_symbol(ctx, "x");
  }

  lexicon->path.sort = Z3_mk_enumeration_sort(ctx, Z3_mk_string_symbol(ctx, "path4_e"), PATH_VARIANTS, lexicon->path.names, lexicon->path.consts, lexicon->path.testers);

  lexicon->path.token.o = Z3_mk_app(ctx, lexicon->path.consts[0], 0, 0);
  lexicon->path.token.a = Z3_mk_app(ctx, lexicon->path.consts[1], 0, 0);
  lexicon->path.token.b = Z3_mk_app(ctx, lexicon->path.consts[2], 0, 0);
  lexicon->path.token.x = Z3_mk_app(ctx, lexicon->path.consts[3], 0, 0);
...
void lexicon_setup_animas(lexicon_s *lexicon, Z3_context ctx, size_t anima_count) {
  ...
  { // Set the (abstract) anima names
    ...
    for (uint8_t idx = 0; idx < anima_count; ++idx) {
      ...
      lexicon->anima.names[idx] = Z3_mk_string_symbol(ctx, name_buffer);
    }
    free(name_buffer);
  }
  ...
  lexicon->anima.sort = Z3_mk_enumeration_sort(ctx,
                                               ...
                                               lexicon->anima.names,
                                               lexicon->anima.consts,
                                               lexicon->anima.testers);
...
void lexicon_setup_persona(lexicon_s *lexicon, Z3_context ctx) {
  ...
  lexicon->persona.names[0] = Z3_mk_string_symbol(ctx, "persona");

  lexicon->persona.sort = Z3_mk_enumeration_sort(ctx,
                                                 Z3_mk_string_symbol(ctx, "persona_t"),
                                                 1,
                                                 lexicon->persona.names,
                                                 lexicon->persona.consts,
                                                 lexicon->persona.testers);
...
void lexicon_assert_anima_location(const lexicon_s *lexicon, Z3_context ctx, Z3_optimize otz, const situation_s *situation, const uint8_t id) {
  ...
  Z3_ast anima_ast = Z3_mk_app(ctx, lexicon->anima.consts[id], 0, 0);
...
void lexicon_assert_persona_location(const lexicon_s *lexicon, Z3_context ctx, Z3_optimize otz, const situation_s *situation) {
  ...
  Z3_ast persona_ast = Z3_mk_app(ctx, lexicon->persona.consts[0], 0, 0);
