...
class path7_t:
    ...
    def print_path(self, maze: maze_t, model: z3_model_t):
        for r in range(0, maze.rows):
            bvr = z3s_bv8.cast(r)
            for c in range(0, maze.cols):
                bvc = z3s_bv8.cast(c)
            ...
            for col in range(0, maze.cols):
                ...
                if maze.is_path(row, col):
                    ...
                    if row > 0:
                        ...
                        if col < maze.cols - 1 and 0 < row and maze.is_path(row - 1, col + 1):
                    ...
                    if col < maze.cols - 1:
                        ...
                        if col + 2 < maze.cols and maze.is_path(row, col + 2):
                            tile_req_e.append(self.z3_path_v(tile_e) == self.EW)
                        if row + 1 < maze.rows and col + 1 < maze.cols and maze.is_path(row + 1, col + 1):
                            tile_req_e.append(self.z3_path_v(tile_e) == self.SW)
                        if 0 < row and col + 1 < maze.cols and maze.is_path(row - 1, col + 1):
                    ...
                    if row + 1 < maze.rows:
                        ...
                        if row + 2 < maze.rows and maze.is_path(row + 2, col):
                            tile_req_s.append(self.z3_path_v(tile_s) == self.NS)
                        if row + 1 < maze.rows and col + 1 < maze.cols and maze.is_path(row + 1, col + 1):
                            tile_req_s.append(self.z3_path_v(tile_s) == self.NE)
                        if row + 1 < maze.rows and 0 < col and maze.is_path(row + 1, col - 1):
                    ...
                    if col > 0:
                        ...
                        if 0 < col and row + 1 < maze.rows and maze.is_path(row + 1, col - 1):
